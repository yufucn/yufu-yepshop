 ...
 */
@Repository
public interface RequirementCommentDAO extends CrudRepository<RequirementCommentDO, Long>,
        JpaSpecificationExecutor<RequirementCommentDO> {

    @Transactional
    @Modifying(clearAutomatically = true)
    @Query(value = "update #{#entityName} set total_reply=IFNULL(total_reply,0) + ?2 where id = ?1", nativeQuery = true)
